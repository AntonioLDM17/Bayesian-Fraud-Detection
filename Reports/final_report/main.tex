...
\title{Detección de fraude basada en incertidumbre con redes bayesianas: de la predicción a la decisión}
...
\begin{abstract}
La detección de fraude es un problema de decisión bajo incertidumbre en el que la automatización total puede ser arriesgada. En este trabajo proponemos un enfoque que combina modelos deterministas con una red neuronal bayesiana para transformar probabilidades predictivas en una política de triaje operativo (\textsc{Accept}, \textsc{Review}, \textsc{Block}).
...
Sea $\mathcal{D}=\{(x_i,y_i)\}_{i=1}^{N}$, con $x_i \in \mathbb{R}^{d}$ y $y_i \in \{0,1\}$,
donde $y_i=1$ indica fraude. El objetivo no es solo estimar
$p(y_i=1\mid x_i,\mathcal{D})$
...
\]
En la implementación se aproxima mediante Monte Carlo sobre la guía:
\[
\hat{p}(x^\star)=
...
La incertidumbre predictiva usada por el sistema es la desviación estándar de esas probabilidades muestrales:
\[
u(x^\star)=
\sqrt{
\frac{1}{S-1}
...
Esta cantidad no es una descomposición formal aleatoric/epistemic, pero sí una señal operativa consistente de dispersión posterior que el sistema aprovecha para abstención selectiva y triaje. En términos prácticos, una incertidumbre alta significa que distintas muestras del posterior inducen probabilidades de fraude sustancialmente distintas para una misma transacción. Esa variabilidad es interpretable como una falta de estabilidad de la decisión automática: aunque la probabilidad media pueda ser moderada o incluso alta, el sistema no debería comportarse igual que en un caso donde distintas muestras del posterior coinciden de forma consistente. Por ello, en este trabajo la incertidumbre no se interpreta como una estadística decorativa, sino como una señal de fiabilidad útil para controlar el grado de automatización.

El repositorio no usa un umbral fijo universal. Para cada modelo se selecciona en validación el umbral que maximiza F1:
\[
\tau^\star=
\arg\max_{\tau\in[0,1]}
\mathrm{F1}\bigl(y,\mathbb{I}[\hat{p}(x)\ge \tau]\bigr),
\]
y ese mismo umbral se reutiliza sin reoptimización en test. Esto evita \emph{test leakage} y fija un punto de operación comparable entre modelos. En el caso de la BNN, el umbral validado y reutilizado en test es
$\tau^\star_{\mathrm{BNN}} = 0.8999524712562561.$
...
d(x)=
\begin{cases}
\textsc{Block}, & \hat{p}(x)\ge \tau_{\mathrm{block}} \;\wedge\; u(x)\le \tau_u,\\[4pt]
\textsc{Review}, & \hat{p}(x)\ge \tau^\star \;\vee\; u(x)>\tau_u,\\[4pt]
...
Además de la BNN, comparamos \emph{Logistic Regression}, \emph{Random Forest} y \emph{XGBoost} como baselines deterministas fuertes. La Tabla~\ref{tab:hyperparams} resume únicamente los hiperparámetros que más condicionan el comportamiento probabilístico y computacional del sistema. Se omiten parámetros secundarios de implementación para priorizar la interpretación experimental.
